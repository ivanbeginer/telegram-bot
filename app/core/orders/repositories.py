...
class OrderRepository:
    ...
    async def create_order(self,user_id)->int:
        async with self.database.session() as session:
            stmt = insert(Order).values(user_id=user_id,status=OrderStatusEnum.unlisted)
            order_id = await session.execute(stmt)
            await session.commit()
            return order_id.scalar()
    async def get_order_by_id(self,order_id:int)->Order|None:
        async with self.database.session() as session:
            stmt = select(Order).where(Order.id==order_id).options(joinedload(Order.products).joinedload(OrderedProduct.product))
            return await(session.scalar(stmt))

    async def get_active_order_for_user(self,user_id:int)->Order|None:
        async with self.database.session() as session:
            stmt = select(Order).where(and_(Order.user_id==user_id,Order.status.in_([OrderStatusEnum.unlisted,OrderStatusEnum.ordered]))).options(joinedload(Order.products).joinedload(OrderedProduct.product))
            return await session.scalar(stmt)
    ...
    async def add_product_to_order(self,order_id:int,product_id:int)->None:
        async with self.database.session() as session:
            ...
            upsert_stmt = insert_stmt.on_conflict_do_update(index_elements=[OrderedProduct.order_id,OrderedProduct.product_id],set_={'amount':OrderedProduct.amount+1})
